...
let environments: immutable.Map<string, string | Environment | undefined>;
let logger: Logger;
...
beforeEach(async () => {
    environments = await getEnvs(process.env, Envs);
    logger = createLogger(environments.get("NODE_ENV") as Environment, environments.get("SERVICE_NAME") as string, LogLevel.DEBUG);
});

describe("sync function", () => {
    it("should sync user", async () => {
        const key = sprintf("%s_users.json", moment().format("YYYY-MM-DD-HH:mm:ss"))
        const { s3, tw } = getClientsFromEnvs(environments)

        const result = sync(logger, tw, { Bucket: environments.get("S3_BUCKET_NAME") as string, Key: key }, s3)(validJSON());
        ...
        expect(p.Bucket).toBe(environments.get('S3_BUCKET_NAME'));
    });
});